...
trait CustomAssertionsTrait
{
	 ...
	 */
	private function assertJsonFileWithinJsonFile(string $expectedFile, string $actualSubsetFile): void
	{
		$this->assertFileExists($expectedFile);
		$this->assertFileExists($actualSubsetFile);
		$this->assertArraySubsetRecursive(
			(array)json_decode((string)file_get_contents($expectedFile), true, 512, JSON_THROW_ON_ERROR),
	...
	private function assertArraySubsetRecursive(array $expected, array $actual, string $message = '', string $path = '$'): void
	{
		foreach ($expected as $key => $value) {
			$currentPath = is_string($key) && preg_match('/^\w+$/', $key) === 1
				? "{$path}.{$key}"
				: "{$path}[" . var_export($key, true) . "]";

			$this->assertArrayHasKey($key, $actual, ltrim("{$message}\nMissing key at path: {$currentPath}"));

			if (!is_array($value) || !is_array($actual[$key])) {
				$this->assertSame($value, $actual[$key], ltrim("{$message}\nMismatch at path: {$currentPath}"));
				continue;
			}

			$this->assertArraySubsetRecursive($value, $actual[$key], $message, $currentPath);
		}
	}
}
